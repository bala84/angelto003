...
set ENV_FILE=%CONF_DIR%env_test.cmd
...
echo.******************************************************
"F:\Program Files\Microsoft SQL Server\90\Tools\Binn\sqlcmd" -S %SQL_SERVER_NAME% -U %SQL_DB_OWNER_USER% -P %SQL_DB_OWNER_USER_PWD% -i !DC_NAME! -o %LOG_DIR%\!DC_NAME!.log
)
) else (
