:r ./../_define.sql
:setvar dc_number 00107                   
:setvar dc_description "check_ts_type fixed#5"
...
********************************************************************************
1.0    13.03.2008 VLavrentiev  check_ts_type fixed#5
*******************************************************************************/ 
use [$(db_name)]
GO
...
execute sp_addextendedproperty 'MS_Description', 
   'Отправлен на ТО',
   'user', @CurrentUser, 'table', 'CHIS_CONDITION', 'column', 'sent_to'
go


/*==============================================================*/
/* Index: i_sent_to_chis_condition	                        */
/*==============================================================*/
create index i_sent_to_chis_condition on CHIS_CONDITION (
sent_to DESC
)
on "$(db_name)_IDX"
go
...
if (@p_Result_id is null)
     SELECT @p_Result_id = id, @v_periodicity = periodicity
		...
		(SELECT TOP(1) id, periodicity, tolerance
		FROM dbo.utfVCAR_TS_TYPE_MASTER() as a
       WHERE @p_run >= (((ceiling(@p_run/a.periodicity)-1)*a.periodicity))
  		 AND @p_run > 0 
		and (((ceiling(@p_run/a.periodicity)-1)*a.periodicity) -  a.tolerance) > 0 
		 AND
		EXISTS
		(select 1 from dbo.utfVCAR_CAR() as b
		   where b.id = @p_car_id
		...
		and c.last_ts_type_master_id = a.id
		and c.sent_to = 'Y'
		and c.run >= ((ceiling(@p_run/a.periodicity)-1)*a.periodicity) -  a.tolerance
		and c.run <= ((ceiling(@p_run/a.periodicity))*a.periodicity)
		)
  ...
  set nocount on
	...
	insert into dbo.CHIS_CONDITION 
     (date_created, action, car_id, ts_type_master_id, employee_id
	  ...
	  , fuel_start_left, fuel_end_left, edit_state, sent_to, sys_comment, sys_user_created, sys_user_modified)
	   values
	...
	,@p_fuel_start_left, @p_fuel_end_left, @p_edit_state, @p_sent_to, @p_sys_comment, @p_sys_user, @p_sys_user)
