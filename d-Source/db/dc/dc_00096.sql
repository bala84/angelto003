:r ./../_define.sql
:setvar dc_number 00096
:setvar dc_description "condition save fixed"
...
********************************************************************************
1.0    07.03.2008 VLavrentiev  condition save fixed
*******************************************************************************/ 
use [$(db_name)]
GO
...
values(100, 'ACTION_INSERT', 'Действие вставки')
go


insert into dbo.CSYS_CONST (id, name, description)
values(101, 'ACTION_UPDATE', 'Действие редактирования')
go



insert into dbo.CSYS_CONST (id, name, description)
values(102, 'ACTION_DELETE', 'Действие удаления')
go
...
begin
   ...
   declare @t_run table (run decimal)
  ...
  -- надо править существующий
	begin
		...
		output inserted.run into @t_run		
		where ID = @p_id
		

		set @v_action = dbo.usfConst('ACTION_UPDATE')

		select @p_run = run from @t_run
...
ALTER procedure [dbo].[uspVCAR_CONDITION_DeleteById]
/******************************************************************************
**
** Author : VLavrentiev
**
...
begin
   ...
   set @v_action = dbo.usfConst('ACTION_DELETE')	
	...
	where id = @p_id
  	...
  	exec @v_Error = 
        dbo.uspVHIS_CONDITION_SaveById
    		...
    		,@p_action						= @v_action
			,@p_car_id						= 0
    		,@p_ts_type_master_id			= 0
			,@p_last_ts_type_master_id		= 0
    		,@p_employee_id					= 0
    		,@p_run							= 0
    		,@p_last_run					= 0
