...
:setvar dc_number 00352
:setvar dc_description "car new fields added"
...
********************************************************************************
1.0    31.07.2008 VLavrentiev  reports pw trailer fixed
*******************************************************************************/ 
use [$(db_name)]
GO
...
add car_passport varchar(100)
go

declare @CurrentUser sysname
select @CurrentUser = user_name()
execute sp_addextendedproperty 'MS_Description', 
   '№ удостоверения автомобиля',
   'user', @CurrentUser, 'table', 'CCAR_CAR', 'column', 'car_passport'
go

alter table dbo.CCAR_CAR
add driver_list_type_id numeric(38,0)
go
...
execute sp_addextendedproperty 'MS_Description', 
   ...
   'user', @CurrentUser, 'table', 'CCAR_CAR', 'column', 'driver_list_type_id'
go

create index ifk_drv_list_type_id_ccar_car on dbo.CCAR_CAR(driver_list_type_id)
go
 

update 
dbo.CDRV_DRIVER_LIST_TYPE
set  short_name = 'Форма №3'
	,full_name = 'Форма №3'
where id = 60
go
...
(
	SELECT a.id
		  ...
		  ,k.short_name as driver_list_type_sname
      FROM dbo.CCAR_CAR as a
		...
		LEFT OUTER JOIN dbo.CDRV_DRIVER_LIST_TYPE as k on k.id = a.driver_list_type_id
)
GO
