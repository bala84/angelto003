...
:setvar dc_number 00284
:setvar dc_description "car summary fixed"
...
********************************************************************************
1.0    04.06.2008 VLavrentiev  car summary fixed
*******************************************************************************/ 
use [$(db_name)]
GO
  ...
  set  @v_value_fuel_gived_id = dbo.usfConst('CAR_FUEL_GIVED_AMOUNT')
   ...
   (SELECT 
	...
	group by
		 state_number
		,organization_id
		,organization_sname) as b
	order by organization_sname, state_number
...
values (78, 'CAR_FUEL_RETURN_AMOUNT', 'Количество возвращенного топлива')
go
...
create PROCEDURE [dbo].[uspVREP_CAR_FUEL_RETURN_Calculate]
/******************************************************************************
**
** Author : VLavrentiev
**
**-----------------------------------------------------------------------------
** Description:
** Процедура должна подсчитывать данные для отчетов о возврате топлива по автомобилям  
**
**  Входные параметры:
**------------------------------------------------------------------------------
** Version: Date:      Author:		Comments:
...
GRANT EXECUTE ON [dbo].[uspVREP_CAR_FUEL_RETURN_Calculate] TO [$(db_app_user)]
GO

GRANT VIEW DEFINITION ON [dbo].[uspVREP_CAR_FUEL_RETURN_Calculate] TO [$(db_app_user)]
GO
			  ...
			  + day_29 + day_30 + day_31
				from dbo.utfVREP_CAR_DAY() as b
						 where value_id = @v_value_fuel_cnmptn_id
						   and b.month_created = a.month_created
						   and b.value_id = a.value_id
						   and b.car_id = a.car_id))), 0) as fuel_consumption
		 ,isnull((convert(decimal(18,0)
		    ,(select 
				day_1 + day_2 + day_3 + day_4 + day_5 + day_6 + day_7
			  +	day_8 + day_9 + day_10 + day_11 + day_12 + day_13 + day_14
			...
			from dbo.utfVREP_CAR_DAY() as b
						 where value_id = @v_value_run_id
						   and b.month_created = a.month_created
						   and b.value_id = a.value_id
						   and b.car_id = a.car_id))), 0) as run
		 ,isnull((select TOP(1) fuel_start_left from dbo.utfVREP_CAR_DAY() as c
												  where c.month_created >= a.month_created
												   and  c.value_id = a.value_id
												   and  c.car_id = a.car_id
		...
		- 
			...
			from dbo.utfVREP_CAR_DAY() as b
						 where value_id = @v_value_fuel_return_id
						   and b.month_created = a.month_created
						   and b.value_id = a.value_id
						   and b.car_id = a.car_id))), 0)) as fuel_gived
	,organization_id
	,organization_sname
	FROM dbo.utfVREP_CAR_DAY() as a
	where month_created between  dbo.usfUtils_TimeToZero(@p_start_date) 
