...
:setvar dc_number 00165                  
:setvar dc_description "new report added"
...
********************************************************************************
1.0    08.04.2008 VLavrentiev  new report added
*******************************************************************************/ 
use [$(db_name)]
GO
...
values(62, 'CAR_KM_AMOUNT', 'Суммарный пробег автомобиля')
go
...
CREATE PROCEDURE [dbo].[uspVREP_CAR_KM_Calculate]
/******************************************************************************
**
** Author : VLavrentiev
**
**-----------------------------------------------------------------------------
** Description:
** Процедура должна подсчитывать данные для отчетов о автомобилям по суммарному 
** пробегу
**
**  Входные параметры:
**------------------------------------------------------------------------------
...
GRANT EXECUTE ON [dbo].[uspVREP_CAR_KM_Calculate] TO [$(db_app_user)]
GO

GRANT VIEW DEFINITION ON [dbo].[uspVREP_CAR_KM_Calculate] TO [$(db_app_user)]
GO
...
drop constraint crep_car_hour_pk
go

alter table dbo.CREP_CAR_HOUR
alter column car_id numeric(38,0) not null
go

alter table dbo.CREP_CAR_HOUR
add constraint crep_car_hour_pk
primary key (day_created, car_id, value_id)
on $(fg_idx_name)
go
...
drop constraint crep_car_day_pk
go
...
alter table dbo.CREP_CAR_DAY
add constraint crep_car_day_pk
primary key (month_created, car_id, value_id)
on $(fg_idx_name)
go


alter table dbo.CREP_CAR_MONTH
drop constraint crep_car_month_pk
go
...
alter table dbo.CREP_CAR_MONTH
add constraint crep_car_month_pk
primary key (year_created, car_id, value_id)
on $(fg_idx_name)
go
