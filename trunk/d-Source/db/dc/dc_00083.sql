:r ./../_define.sql
:setvar dc_number 00083
:setvar dc_description "last_ts_type_master_id added"
...
********************************************************************************
1.0    05.03.2008 VLavrentiev  last_ts_type_master_id added
*******************************************************************************/ 
use [$(db_name)]
GO
...
add last_ts_type_master_id numeric(38,0)
go
...
execute sp_addextendedproperty 'MS_Description', 
   'Последний вид ТО',
   'user', @CurrentUser, 'table', 'CCAR_CONDITION', 'column', 'last_ts_type_master_id'
go
...
begin
  ...
  if (@p_id is null)
    begin
	   ...
	   ,run, last_run, last_ts_type_master_id, speedometer_start_indctn, speedometer_end_indctn, sys_comment, sys_user_created, sys_user_modified)
	   values
	(@p_car_id, dbo.usfVCAR_CONDITION_Check_ts_type(@p_run, @p_car_id, @p_last_ts_type_master_id), @p_employee_id
	,@p_run, @p_last_run, @p_last_ts_type_master_id, @p_speedometer_start_indctn, @p_speedometer_end_indctn, @p_sys_comment, @p_sys_user, @p_sys_user)
