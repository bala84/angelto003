:r ./../_define.sql
:setvar dc_number 00077
:setvar dc_description "run save fixed"
...
********************************************************************************
1.0    03.03.2008 VLavrentiev  run save fixed
*******************************************************************************/ 
use [$(db_name)]
GO
...
begin
  ...
  -- надо править существующий
    ...
    -- ≈сли мы редактировали начальный пробег удостоверимс€, что мы прибавим дельту
  -- if (@p_last_begin_run <> 0) 
 --	set @p_begin_run = @p_begin_run  - @p_last_begin_run



    -- ≈сли мы изменили начальное показание, то мы должны откорректировать пробег и все показани€
...
-- ”бедимс€, что при отсутствии изменений мы не увеличим пробег
 if (@p_condition_id is not null) and (@p_last_run = 0)
  set @p_run = 0
