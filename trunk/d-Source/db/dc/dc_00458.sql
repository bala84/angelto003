...
:setvar dc_number 00458
:setvar dc_description "warehouse gd type added"
...
********************************************************************************
1.0   17.04.2009 VLavrentiev   warehouse gd type added
*******************************************************************************/ 
use [$(db_name)]
GO
...
select 
	...
	,case 
	      when @p_type = 1 then convert(decimal(18,2), avg(price))
	      when @p_type = 2 then null 
	 end as price
	,sum(total) as total
	,sum_demanded
	,demand_number
...
(select a.date_created
	 ...
	 , d.state_number
	 , case when @p_type = 1 then e.short_name
			when @p_type = 2 then e2.short_name
		end as short_name
  ...
  from dbo.cwrh_wrh_order_master as a
   ...
   left outer join dbo.cwrh_good_category_type as e2 on e.good_category_type_id = e2.id
where a.sys_status = 1
  and c.sys_status = 1
  and g.sys_status = 1
  and h.sys_status = 1
  and isnull(e2.sys_status,1) = 1
...
(select 
	 ...
	 , a.state_number
	 , case when @p_type = 1 then e2.short_name
			when @p_type = 2 then e4.short_name
		end as short_name
...
(select top(5) 
  ...
  select top(5) a2.id
   ...
   select top(5) a2.id
		...
		join dbo.cwrh_wrh_demand_detail as d2 on c2.id = d2.wrh_demand_master_id
		join dbo.cwrh_good_category as e2 on d2.good_category_id = e2.id
		left outer join dbo.cwrh_good_category_type as e4 on e2.good_category_type_id = e4.id
...
group by date_created
	...
	,sum_demanded
	,reason    
order by org_name, kind_name, state_number, sum(sum_demanded) desc, number, date_created, demand_number, short_name
