...
:setvar dc_number 00364
:setvar dc_description "driver list car_state added"
...
********************************************************************************
1.0    19.08.2008 VLavrentiev  driver list car_state added
*******************************************************************************/ 
use [$(db_name)]
GO
...
values(302, 'ON_DUTY', 'На линии')
go
insert into dbo.csys_const(id, name, description)
values(303, 'IN_PARK', 'На стоянке')
go
...
if (@p_driver_list_state_id = dbo.usfConst('LIST_OPEN'))
 update dbo.CCAR_CAR
	set car_state_id = dbo.usfConst('ON_DUTY') 
	where id = @p_car_id
else
  update dbo.CCAR_CAR
	set car_state_id = dbo.usfConst('IN PARK') 
	where id = @p_car_id
