...
:setvar dc_number 00156                  
:setvar dc_description "rep day added"
...
********************************************************************************
1.0    02.04.2008 VLavrentiev  rep day added
*******************************************************************************/ 
use [$(db_name)]
GO
...
create FUNCTION [dbo].[usfUtils_DayTo01](@date_with_time datetime)
RETURNS datetime
AS
BEGIN
...
GRANT EXECUTE ON [dbo].[usfUtils_DayTo01] TO [$(db_app_user)]
GO

GRANT VIEW DEFINITION ON [dbo].[usfUtils_DayTo01] TO [$(db_app_user)]
GO
...
create procedure [dbo].[uspVREP_CAR_DAY_SaveById]
/******************************************************************************
**
** Author : VLavrentiev
**
...
begin
	...
	set @p_sys_comment = '-'
	 if (@p_month_created is null)
	  set @p_month_created = dbo.usfUtils_DayTo01(getdate())
	 else
	  set @p_month_created = dbo.usfUtils_DayTo01(@p_day_created)
...
GRANT EXECUTE ON [dbo].[uspVREP_CAR_DAY_SaveById] TO [$(db_app_user)]
GO

GRANT VIEW DEFINITION ON [dbo].[uspVREP_CAR_DAY_SaveById] TO [$(db_app_user)]
GO
...
CREATE trigger [TRIUD_CREP_CAR_DAY]
on  [dbo].[CREP_CAR_HOUR]
...
if (@@rowcount = 1)
begin
--if (update(day_created))

select 
		    @v_month_created = dbo.usfUtils_DayTo01(day_created)
		   ,@v_value_id = value_id
...
ALTER trigger [TRIUD_CREP_CAR_DAY]
on  [dbo].[CREP_CAR_HOUR]
AFTER INSERT, UPDATE, DELETE
as
begin
...
drop trigger TRIUD_CREP_CAR_DAY
go
...
begin
  ...
  set @v_value = @p_hour_0 + @p_hour_1 + @p_hour_2
					 + @p_hour_3 + @p_hour_4 + @p_hour_5 + @p_hour_6
					 + @p_hour_7 + @p_hour_8 + @p_hour_9 + @p_hour_10
					 + @p_hour_11 + @p_hour_12 + @p_hour_13 + @p_hour_14
					 + @p_hour_15 + @p_hour_16 + @p_hour_17 + @p_hour_18
