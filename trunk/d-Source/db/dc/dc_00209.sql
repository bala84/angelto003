...
:setvar dc_number 00209
:setvar dc_description "check ts type fixed#4"
...
********************************************************************************
1.0    20.04.2008 VLavrentiev  check ts type fixed#4
*******************************************************************************/ 
use [$(db_name)]
GO
	...
	begin
	  ...
	  where id = @p_id
		...
		where exists
			(select 1 from
				(select a.id from
						(select run, car_id, last_ts_type_master_id from dbo.chis_condition
																	where id = @p_id) as b
						left outer join dbo.chis_condition as a on a.run = b.run and a.car_id = b.car_id
				where a.last_ts_type_master_id != b.last_ts_type_master_id and a.sent_to = 'Y') as c
			where c.id = dbo.chis_condition.id)
